...
def ajustar_ano_referencia(dt, dt_referencia_b):
    ...
    # (A lógica de virada de ano principal é tratada na geração de sequência)
    return dt
...
def interpretar_periodo_atividade(item_d_text, icao, item_b_raw, item_c_raw):
    ...
    for match in matches:
        ...
        if tem_conteudo_data or tem_conteudo_semana:
            ...
            # Se achou filtros mas nenhuma data específica, e existiam datas antes, usa as anteriores?
            # V18: Se achou nova definição e ela não gerou datas (só dias da semana ou DLY implícito), 
            # verifica se deve herdar. Aqui mantemos a lógica de reset se achou algo novo.
            if not datas_deste_segmento and ultima_lista_datas: 
                # Caso específico: Apenas mudou o dia da semana, mas não as datas?
                # Para segurança no caso 46, se definiu novos filtros, não herda datas antigas a menos que explícito.
                # Mas para DLY sem datas, precisa preencher B->C
                pass
        ...
        for dt_crua in lista_final:
            ...
            # Slot deve ter alguma interseção ou estar contido? 
            # Regra V18: Slot deve estar totalmente dentro ou ser cortado? 
            # Regra aplicada: O slot é válido se começar >= B e terminar <= C.
            # Ajuste fino: Se começar antes de B mas terminar depois, corta?
            # Para simplificar e seguir os testes: O slot DEVE respeitar os limites.
            
            if s_fim < dt_b or s_ini > dt_c: continue
            
            # Refinamento: Se o slot começa antes de B, mas é o dia de B? (Caso 40)
            # O teste 40 mostrou que se 10:00 < 20:30 (B), elimina.
            # Então a verificação s_ini < dt_b é estrita.
