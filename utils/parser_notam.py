...
def interpretar_periodo_atividade(item_d_text, icao, item_b_raw, item_c_raw):
    """
    Versão 18.7: Correção de Duplicidade em Datas com Barra e Pernoites (Cases 48-60)
    """
            ...
            while i < len(tokens):
                ...
                if tok[0].isdigit():
                    ...
                    if "/" in tok: 
                        ...
                        # --- FIX V18.7: Duplicidade em Overnight ---
                        # Se for overnight e forem datas consecutivas (ex: 17/18), pega só a primeira.
                        consecutivos = False
                        try:
                            v1 = int(partes[0])
                            v2 = int(partes[1])
                            if v2 == v1 + 1 or (v1 >= 28 and v2 == 1): # Lógica básica de consecutivo
                                consecutivos = True
                        except: pass
