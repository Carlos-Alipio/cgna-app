...
def criar_data_ajustada(ano_base, mes, dia, dt_ref_b):
    """
    Cria a data e IMEDIATAMENTE ajusta o ano com base no início do NOTAM (B).
    Evita o bug de ajuste tardio que causava duplo incremento.
    """
    try:
        dt = datetime(ano_base, mes, dia)
        # Se a data extraída é JAN/FEB e o NOTAM começa em NOV/DEC, é ano seguinte
        if dt.month < dt_ref_b.month and dt_ref_b.month > 10:
            return dt.replace(year=dt_ref_b.year + 1)
        return dt
    except: return None

def gerar_sequencia_datas(ano_base, mes_ini, dia_ini, mes_fim, dia_fim, dt_ref_b):
    """
    Gera lista de datas entre dois pontos, calculando o ano correto para Início e Fim
    antes de preencher o meio.
    """
    datas = []
    # Cria datas já com o ano corrigido relativo ao B
    dt_start = criar_data_ajustada(ano_base, mes_ini, dia_ini, dt_ref_b)
    dt_end = criar_data_ajustada(ano_base, mes_fim, dia_fim, dt_ref_b)
    
    if not dt_start or not dt_end: return []
    
    # Se, após o ajuste base, o fim ainda for menor que o início (ex: range JAN 10 TIL JAN 05 - raro, ou erro),
    # ou se for uma virada de ano interna específica do range que o ajuste base não pegou:
    if dt_end < dt_start: 
...
def interpretar_periodo_atividade(item_d_text, icao, item_b_raw, item_c_raw):
    """
    Parser V19 (Hotfix Regression)
    Corrigido bug de cálculo de anos em Ranges (Caso 16 e 28).
    """
    ...
    for match in matches:
                ...
                if tok[0].isdigit():
                    if i + 2 < len(tokens) and tokens[i+1] == "TIL":
                        ...
                        if alvo[0].isdigit():
                            ...
                            # Passamos dt_b para calcular o ano correto JÁ AQUI
                            datas_deste_segmento.extend(gerar_sequencia_datas(contexto_ano, contexto_mes, dia_start, contexto_mes, dia_end, dt_b))
                            i += 3; continue
                            ...
                            if i + 3 < len(tokens) and tokens[i+3][0].isdigit():
                                ...
                                # Passamos dt_b para calcular o ano correto JÁ AQUI
                                datas_deste_segmento.extend(gerar_sequencia_datas(contexto_ano, contexto_mes, dia_start, mes_dest, dia_end, dt_b))
                                contexto_mes = mes_dest; i += 4; continue

                    if "/" in tok: 
                        partes = tok.split("/")
                        dt = criar_data_ajustada(contexto_ano, contexto_mes, int(partes[0]), dt_b)
                        if dt: datas_deste_segmento.append(dt)
                        if offset_dias == 0 and not is_complex: 
                             dt2 = criar_data_ajustada(contexto_ano, contexto_mes, int(partes[1]), dt_b)
                             if dt2: datas_deste_segmento.append(dt2)
                        i += 1; continue
                    else:
                        dt = criar_data_ajustada(contexto_ano, contexto_mes, int(tok), dt_b)
                        if dt: datas_deste_segmento.append(dt)
                        i += 1; continue
                i += 1
        ...
        for dt_final in lista_final:
            # REMOVIDO: dt_final = ajustar_ano_referencia(...) 
            # O ajuste agora acontece na criação (criar_data_ajustada), evitando duplo incremento.
            ...
            # Validação Estrita de Bordas (B e C)
