...
# ==============================================================================
# FUNÇÃO PRINCIPAL (V22.0 - LOGIC SWAP FIX)
# ==============================================================================

def interpretar_periodo_atividade(item_d_text, icao, item_b_raw, item_c_raw):
    """
    V22.0: Prioridade de Range sobre Barras. 
    Corrige o bug de "Under-counting" (107 vs 130 slots) em MON/TUE TIL THU/FRI.
    """
    ...
    for match in matches:
        ...
        if tem_conteudo_data or tem_conteudo_semana:
            ...
            while k < len(tokens):
                ...
                # --- ORDEM CORRIGIDA: PRIMEIRO TENTA RANGE, DEPOIS DIAS ISOLADOS ---
                
                # 1. Tenta identificar RANGE (TIL) mesmo se o token atual tiver barra
                is_range_start = False
                if k + 2 < len(tokens) and tokens[k+1] == "TIL":
                    # Identifica início do range
                    idx_tok = None
                    if tok in WEEK_MAP: 
                        idx_tok = WEEK_MAP[tok]
                    elif "/" in tok:
                        # Se for MON/TUE TIL..., pega o primeiro dia (MON)
                        sub = tok.split("/")
                        if sub[0] in WEEK_MAP: idx_tok = WEEK_MAP[sub[0]]
                    
                    if idx_tok is not None:
                        alvo_tok = tokens[k+2]
                        idx_alvo = None
                        
                        # Identifica fim do range
                        if alvo_tok in WEEK_MAP:
                            idx_alvo = WEEK_MAP[alvo_tok]
                        elif "/" in alvo_tok:
                            # Se for ...TIL THU/FRI, e for overnight, pega o primeiro (THU)
                            sub = alvo_tok.split("/")
                ...
                # 2. Se não for range, processa como dia isolado (com ou sem barra)
                ...
                # Se não for dia da semana, avança
                k += 1
            
            # --- PROCESSAMENTO DE DATAS NUMÉRICAS ---
            i = 0 
            while i < len(tokens):
                tok = tokens[i]
                # Pula tokens de semana ou irrelevantes
        ...
        for dt_crua in datas_deste_segmento:
            ...
            if tipo_match == 'continuo':
                ...
                if dia_alvo < dt_final.day:
                    mes_fim_calc += 1
                    if mes_fim_calc > 12: mes_fim_calc = 1; ano_fim_calc += 1
